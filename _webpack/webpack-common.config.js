 ...
 *    Last modified: 7/23/23, 10:54 PM
...
const basePath = path.join(process.env.MOONSPHERE_BASE_PATH, "tailwind");
...
const postCssConfig = {
    postcssOptions: {
        plugins: [
            ...
            require("tailwindcss")(path.resolve(__dirname, "..", "_tailwind.config.js")),
        ],
    },
};

const commonWebpackConfig = ({ landingPageBaseUrl, clientBaseUrl, cdnBaseUrl, isProdMode }) => ({
    context: path.resolve(__dirname, ".."),
    entry: {
        "polyfills": [ path.resolve(__dirname, "..", "src", "polyfills.ts") ],
        "main": [ path.resolve(__dirname, "..", "src", "main.ts") ],
        "fonts": [ path.resolve(__dirname, basePath, "common", "fonts.scss") ],
        "base": [ path.resolve(__dirname, basePath, "common", "base.scss") ],
        "components": [ path.resolve(__dirname, basePath, "common", "components.scss") ],
        "custom-components": [ path.resolve(__dirname, "..", "src", "styles", "combined.scss") ],
        "vendor": [ path.resolve(__dirname, basePath, "common", "vendor.scss") ],
    },
    output: {
        path: path.resolve(__dirname, "..", "dist"),
        filename: `js/__msph.[${isProdMode ? "contenthash:10" : "name"}].bundle.js`,
        chunkFilename: `js/__msph.[${isProdMode ? "contenthash:10" : "name"}].chunk.js`,
        clean: true,
        publicPath: "/",
    },
    ...
    plugins: [
        ...
        new MiniCssExtractPlugin({
            filename: `css/__msph.[${isProdMode ? "contenthash:10" : "name"}].bundle.css`,
            chunkFilename: `css/__msph.[${isProdMode ? "contenthash:10" : "name"}].chunk.css`,
        }),
        ...
        new AngularWebpackPlugin({
            tsconfig: path.resolve(__dirname, "..", "_tsconfig", "tsconfig.app.json"),
