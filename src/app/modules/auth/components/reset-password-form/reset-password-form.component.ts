...
import { IResetPassword1stStageFormModel, IResetPassword2ndStageFormModel } from "~/auth-mod/models/reset-password-form.model";
...
export class ResetPasswordFormComponent {
    ...
    constructor(
        ...
        this.resetPassword1stStageForm = new FormGroup({
            usernameOrEmailAddress: new FormControl("", [ Validators.required, Validators.pattern(regex.USERNAME_OR_EMAIL) ]),
        });
        this.resetPassword2ndStageForm = new FormGroup({
            token: new FormControl("", [ Validators.required, Validators.pattern(regex.OTA_TOKEN), exactLengthValidator(10) ]),
        });
    };

    onSubmitResetPassword1stStage(): void {
        const formData = this.resetPassword1stStageForm.getRawValue() as IResetPassword1stStageFormModel;
        // on next
    ...
    async onSubmitResetPassword2ndStage(): Promise<void> {
        const formData = this.resetPassword2ndStageForm.getRawValue() as IResetPassword2ndStageFormModel;
        // on next
        console.log(formData);
