...
import { IRegister1stFormModel, IRegister2ndFormModel } from "~/auth-mod/models/register-form.model";
...
export class RegisterFormComponent implements AfterViewInit {
    ...
    @ViewChild("passwordStrengthMeter") passwordStrengthMeterComponent!: PasswordStrengthMeterComponent;

    constructor(
        private readonly _birthDateValidator: BirthDateValidator,
    ) {
        this.register1stStageForm = new FormGroup({
            username: new FormControl("", [ Validators.required, Validators.minLength(3), Validators.pattern(regex.USERNAME) ]),
            emailAddress: new FormControl("", [ Validators.required, Validators.email ]),
            password: new FormControl("", [ Validators.required, Validators.pattern(regex.PASSWORD) ]),
            confirmedPassword: new FormControl("", [ Validators.required ]),
            birthDate: new FormControl({ day: null, month: null, year: null }, this._birthDateValidator.validate())
    ...
    get checkIfFormIsInvalid(): boolean {
        ...
        return this.register1stStageForm.invalid || this.register2ndStageForm.invalid || !agreeTermsControl.value;
    };
    ...
    onSubmitRegisterForm(): void {
        const data1stPart = this.register1stStageForm.getRawValue() as IRegister1stFormModel;
        const data2ndPart = this.register2ndStageForm.getRawValue() as IRegister2ndFormModel;
        // next
