...
export class AuthSingleSelectSpinnerComponent implements OnChanges, AfterViewInit {
    ...
    @Output() persistElement: EventEmitter<ISpinnerListElementType | null> = new EventEmitter();
    ...
    onClickOnDocument(event: MouseEvent): void {
        ...
        if (!this.listElements.find(e => e.value === this.inputElement.nativeElement.value)) {
    ...
    handleFilterElements(phrase: string): void {
        ...
        this.filteredList = this.listElements
            .filter(v => v.value.toLowerCase().includes(phrase.toLowerCase()) || phrase === "");
    };
